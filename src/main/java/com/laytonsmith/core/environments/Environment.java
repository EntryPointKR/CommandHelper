...
/**
 * This is a generic environment storage class. Environments cannot be removed from
 * the environment, but the environment can be cloned, and added to at that time.
 * Classes don't need to strictly know what environments are contained within the
 * class, but if they are expecting a specific environment type, they can pull
 * it out, and gain the advantage of a specific type of exception being thrown
 * if it is not, and if it is, getting type safety on the environment itself.
 * <p>
 * "Sub environments" must implement {@link EnvironmentImpl}, which ensures that
 * they are cloneable.
 */
