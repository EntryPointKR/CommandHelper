...
public enum Signals implements SignalType {
     ...
     * (such as setitimer) elapses. SIGALRM is sent when real or clock time
     * elapses. SIGVTALRM is sent when CPU time used by the process elapses.
     * SIGPROF is sent when CPU time used by the process and by the system on
     ...
     * conditions that lead to the signal being raised are, for example,
     ...
     * used by a child process after its termination without an explicit call to
     ...
     * arithmetic operation, such as division by zero (the FPE stands for
     ...
     * later resumption. Cannot be caught or ignored.
     */
     ...
     * releasing resources and saving state if appropriate. It should be noted
     * that SIGINT is nearly identical to SIGTERM.
     */
     ...
     * occurs (meaning it has been polled).
     */
